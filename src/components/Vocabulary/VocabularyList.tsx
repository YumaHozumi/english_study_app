...
export function VocabularyList({ initialWords, isAuthenticated }: VocabularyListProps) {
        ...
        if (session?.user) {
            startTransition(async () => {
                try {
                    ...
                    setDeletedWord(deleted as WordEntry);
                    const entries = await getVocabularyEntries();
                    setWords(entries as WordEntry[]);
                } catch (error) {
        ...
        <div className="p-4 max-w-[600px] mx-auto">
            <header className="mb-6">
                ...
                <p className="text-[var(--text-secondary)] text-sm">
                    ...
                    {!isAuthenticated && words.length > 0 && <span className="ml-2 text-orange-500">⚡ Local only</span>}
                </p>
            </header>
